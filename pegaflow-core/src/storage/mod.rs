...
pub struct StorageConfig {
    ...
    /// Max blocks allowed in remote fetch state (backpressure). None = disabled.
    pub max_remote_fetch_blocks: Option<usize>,
}
...
impl StorageEngine {
    pub(crate) fn new_with_config(
        ...
        numa_nodes: &[NumaNode],
        metaserver_registrar: Option<Arc<MetaServerRegistrar>>,
        remote_fetch_fn: Option<remote_fetch::RemoteFetchFn>,
    ) -> Arc<Self> {
        ...
        let max_remote_fetch_blocks = config.max_remote_fetch_blocks;
...
mod tests {
    ...
    fn make_engine() -> Arc<StorageEngine> {
        StorageEngine::new_with_config(1 << 20, false, StorageConfig::default(), &[], None, None)
    }
    ...
    async fn allocate_bounded_reclaim_terminates() {
        ...
        let storage =
            StorageEngine::new_with_config(4096, false, StorageConfig::default(), &[], None, None);
    ...
    fn make_engine_with_transfer_lock() -> Arc<StorageEngine> {
        ...
        StorageEngine::new_with_config(1 << 20, false, config, &[], None, None)
    }
