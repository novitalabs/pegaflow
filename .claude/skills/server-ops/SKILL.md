...
| `--metaserver-addr` | — | MetaServer gRPC address; enables registration + remote fetch. Requires `--advertise-addr` and `--rdma-nic` |
| `--advertise-addr` | — | Advertised addr for metaserver. Required when `--metaserver-addr` is set |
