...
//! PushBlocks (holder RDMA-WRITEs into requester slabs) -> SealedBlock
//! rebuild, and optionally loads the blocks to GPU and verifies every byte.
//!
//! Holder (node A):
...
fn register_ranks(engine: &PegaEngine, shape: &Shape) -> Vec<RankBuffers> {
    ...
    let layer_ids: Vec<usize> = (0..shape.layers).collect();
    (0..shape.tp)
        .map(|rank| {
