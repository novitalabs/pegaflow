...
class SchedulerConnector:
    ...
    def __init__(
        ...
        if context.num_linear_state_cache_slots:
            self._linear_state_cache = LinearStateCache(context.num_linear_state_cache_slots)
    ...
    def _finish_cache_lookup(
        ...
        *,
        ...
        if probe.recurrent_hold is not None or probe.local_recurrent is not None:
            ...
            # the token budget cuts inside the reconciled span, fall back to
            # the newest earlier committed checkpoint.
            ...
            if pending_probe is not None and pending_probe.local_recurrent is not None:
                try:
                    destinations = tuple(
                        (
                            group_index,
                            self._recurrent_load_destination(
                                group_index, block_ids_by_group[group_index]
                            ),
                        )
    ...
    def _build_connector_meta(self, scheduler_output: "SchedulerOutput") -> PegaConnectorMetadata:
        # Keep deferred/load work in place until metadata construction succeeds.
        ready_save_intents = {
            req_id: self._wrap_local_recurrent_save(req_id, intent)
            for req_id, intent in self._deferred_save_intents.items()
        }
        ...
        # Mutate queues and lifecycle only after metadata construction succeeds.
    ...
    def _wrap_local_recurrent_save(self, req_id: str, intent: SaveIntent) -> SaveIntent:
        """Move #441's single committed recurrent row into one compound slot."""
        if self._linear_state_cache is None:
            return intent

        recurrent = sorted(self._cache_groups.recurrent_group_indices)
        positions = []
        sources = []
        for group_index in recurrent:
            row = intent.block_ids_by_group[group_index]
            non_null = [(position, block_id) for position, block_id in enumerate(row) if block_id]
            if not non_null:
                positions.append(None)
                continue
            if len(non_null) != 1:
                raise RuntimeError(
                    f"req {req_id} final HMA save has multiple recurrent checkpoints "
                    f"for group {group_index}"
                )
            position, block_id = non_null[0]
            positions.append(position)
            sources.append((group_index, block_id))

        non_null_positions = {position for position in positions if position is not None}
        if not non_null_positions:
            return SaveIntent(
                block_ids_by_group=tuple(
                    (0,) * len(intent.block_hashes) if index in recurrent else group
                    for index, group in enumerate(intent.block_ids_by_group)
                ),
                block_hashes=intent.block_hashes,
            )
        if len(non_null_positions) != 1 or any(position is None for position in positions):
            raise RuntimeError(
                f"req {req_id} recurrent groups do not share one committed checkpoint position"
            )
        if self._local_saves_by_req.get(req_id):
            raise RuntimeError(f"req {req_id} already has a local recurrent save reservation")

        position = non_null_positions.pop()
        block_hash = intent.block_hashes[position]
        slot_ref = self._linear_state_cache.reserve(block_hash)
        local_saves: tuple[LocalRecurrentSave, ...] = ()
        if slot_ref is not None:
            save = LocalRecurrentSave(target=slot_ref, source_block_ids=tuple(sources))
            save_ref = LocalSaveRef.from_save(req_id, save)
            self._pending_local_saves[save_ref] = slot_ref
            self._local_saves_by_req[req_id] = {save_ref}
            local_saves = (save,)
        ...
        return SaveIntent(
            ...
            block_hashes=intent.block_hashes,
            local_recurrent_saves=local_saves,
        )
    ...
    def _recurrent_load_destination(
        self,
        group_index: int,
        block_ids: tuple[int, ...],
    ) -> int:
        try:
            state_index = self._cache_groups.recurrent_running_state_index(
                group_index, len(block_ids)
            )
        except RuntimeError as exc:
            raise RuntimeError(f"recurrent group {group_index} has no load destination") from exc
        destination = block_ids[state_index]
        if destination == 0:
            raise RuntimeError(
                f"recurrent group {group_index} running-state destination is the null block"
            )
        return destination
    ...
    def _load_block_ids_by_group(
        ...
        for group_index, block_ids in enumerate(block_ids_by_group):
            if group_index in self._cache_groups.recurrent_group_indices:
                destination = self._recurrent_load_destination(group_index, block_ids)
                result.append((None,) * (num_load_blocks - 1) + (destination,))
                continue
            if len(block_ids) < end_block_idx:
                raise RuntimeError(
                    f"load block mismatch: start={start_block_idx} count={num_load_blocks} "
                    f"group={group_index} available={len(block_ids)}"
                )
            result.append(block_ids[start_block_idx:end_block_idx])
        return tuple(result)

    def update_connector_output(self, connector_output: "KVConnectorOutput") -> None:
        # ACKs and load pins must be settled before finished-sending cleanup can
        # discard the request-owned transaction indexes.
    ...
    def _apply_local_save_acks(self, metadata: PegaWorkerMetadata) -> None:
        ...
        for ref, count in metadata.succeeded.items():
            ...
            if total == self._ctx.tp_size:
                ...
                logger.info(
                    "[PegaKVConnector] local recurrent checkpoint committed: "
                    "req=%s slot=%d generation=%d hash=%s",
                    ref.req_id,
                    ref.slot,
                    ref.generation,
                    ref.block_hash.hex(),
