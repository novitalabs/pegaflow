...
class PegaRdmaV1Handshake:
    status: str
    has_metadata: bool
    @property
    def metadata(self) -> bytes | None: ...

class PegaRdmaV1Engine:
    def __init__(self, *, nics: list[str], qps_per_peer: int = 4) -> None: ...
    def register_memory(self, regions: list[dict[str, int]]) -> None: ...
    def unregister_memory(self, addrs: list[int]) -> None: ...
    def prepare_handshake(self, remote_addr: str) -> PegaRdmaV1Handshake: ...
    def accept_handshake(self, remote_addr: str, remote_metadata: bytes) -> bytes: ...
    def finish_handshake(self, remote_addr: str, remote_metadata: bytes) -> None: ...
    def abort_handshake(self, remote_addr: str) -> None: ...
    def register_local_blocks(self, nixl_handle: int, blocks: list[tuple[int, int, int]]) -> None: ...
    def unregister_local_blocks(self, nixl_handle: int) -> None: ...
    def register_remote_blocks(
        self, engine_id: str, tp_rank: int, blocks: list[tuple[int, int, int]]
    ) -> None: ...
    def unregister_remote_blocks(self, engine_id: str, tp_rank: int | None = None) -> None: ...
    def read_async_indices(
        self,
        remote_addr: str,
    ...
    def release_read(self, handle: int) -> None: ...
    def invalidate_connection(self, remote_addr: str) -> None: ...
