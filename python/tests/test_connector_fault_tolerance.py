...
def test_local_recurrent_pool_oom_is_explicit_and_cleans_partial_state(monkeypatch):
    ...
    recurrent_2.num_speculative_blocks = 0
    kv_cache_config = MagicMock(
        kv_cache_groups=(
    ...
    worker, client, _ = _make_worker(
        kv_cache_config=kv_cache_config,
        num_linear_state_cache_slots=1,
    )
    monkeypatch.setattr(
        "pegaflow.connector.worker._raw_page_view",
        lambda _tensor, _page_bytes: object(),
    )
    oom = __import__("torch").OutOfMemoryError("oom")
    monkeypatch.setattr(
        "pegaflow.connector.worker.torch.empty",
        MagicMock(side_effect=[object(), oom]),
        raising=False,
    )
    empty_cache = MagicMock()
    monkeypatch.setattr("pegaflow.connector.worker.torch.cuda.empty_cache", empty_cache)
