...
底层依赖：
- `sideway = 0.4.3`，作为 verbs 生态入口；`rdma-mummy-sys = 0.2.4`，同时作为 direct dependency 提供 raw verbs FFI；
...
| GPU MR | 不承担 PD GPU path | CUDA device memory 通过 dma-buf fd + upstream `ibv_reg_dmabuf_mr` 注册；缺 dma-buf fd 直接报错 |
...
GPU memory WRITE+IMM 现在走 DMA-BUF MR：`MemoryRegion::new` 对 CUDA device pointer 导出 dma-buf fd，verbs provider 通过 `rdma_mummy_sys::ibv_reg_dmabuf_mr` 注册。缺 dma-buf fd 时直接报错，不再回退到 `ibv_reg_mr(cuda_ptr)`。
...
⚠️ kernel ≥ 5.12 才支持 DMA-BUF MR。第一版不实现 nv_peer_mem fallback；目标机器无法导出 dma-buf fd 时直接报错。
...
1. **DMA-BUF kernel 版本**：部署前确认目标集群 kernel 都 ≥ 5.12；否则 GPU MR 注册会直接失败。
