...
// mr.descriptor() -> MemoryRegionDescriptor (含 (DomainAddress, rkey) 列表), 通过 D→P prefill 请求发给对端
...
第一版实现：
- `MockRdmaPort`：测试专用 double，只记录调用和状态，不进入生产路径；
- `RealRdmaPort`：接 PyO3 `PdRdmaEngine`，不另开 sidecar 路径。

生产路径不再保留 `NoopRdmaPort`。native extension 或 `PdRdmaEngine` 不存在时直接报错，
避免端到端测试误退回到假 RDMA。
...
| `wait_done(req_id)` | 后台 waiter 等 IMM done | 不在 `wait_for_layer_load` 里同步阻塞；worker 通过 `get_finished` 汇总完成 |
...
- D 侧必须在 `register_remote()` 时预注册 IMM counter，不能等 `wait_done()` 再注册；
  否则 P 侧 `push_done()` 发送的 IMM 可能先到，被当作普通 immediate 消费，D 再等
  新 counter 会永久卡住；
- 默认 pin 下 P/D 目前都落在同一组 CPU（cuda:2 上默认 `worker=60, uvm=62`），
  16 MiB 正确性用例约 2.5 Gbps；
- 显式 pin 到 `P=(60,62), D=(64,66)` 后，16 MiB 正确性用例约 24.7 Gbps，
  600 MiB release 用例三次复测约 276.8 / 301.1 / 274.1 Gbps，仍低于
  `pegaflow-cpu-bench-v2` 曾测到的约 372.6 Gbps，但已经进入同一量级；后续接 vLLM
  时要继续看真实请求下的提交粒度和 layer/block batching。

当前 h20-97 回归：
- `pegaflow-cpu-bench-v2 --memory cuda --block-size 4mb --blocks-per-task 96 --tasks 20 --warmup-tasks 5 --cuda-device 0 --domains mlx5_1`
  在单链路 `cuda:0 + mlx5_1` 上达到 p50 8.66 ms，折算约 **372.1 Gbps**，满足
  单 400G HCA 至少 350 Gbps 的阶段目标；
- 小模型 vLLM P/D（P: GPU0, D: GPU1，同 `mlx5_1`）真实 RDMA E2E 可连续返回；
  P 日志显示每个请求 `layers=24`，D 日志显示 `scheduler finished recving`；
- 小模型 vLLM P/D（P: GPU0/`mlx5_1`, D: GPU2/`mlx5_2`）会在 P 侧第一层
  `RDMA WRITE timed out`。这不是 vLLM scheduler 问题，也不是 GPU MR 完全不可用；
  host/cuda bench 和同 HCA vLLM 都能过。当前判断是单机跨 HCA 的 domain/routing
  不能用“每个进程只开本地一个 HCA”的方式假装互通。下一步要做跨 HCA/跨机的多
  domain peer/routing，而不是靠选同 HCA GPU 作为最终解。

### 3.5 控制面

控制面形态参考 vLLM NIXL 的 scheduler/worker 分层，而不是照搬它的 ZMQ metadata
查询。当前实现里：
- scheduler/worker 之间走 vLLM 自己的 connector metadata；
- D worker 在 allocate KV 后构造 `PdHandshake`，通过发给 P 的 prefill HTTP 请求携带
  prompt、producer-side `kv_transfer_params`、layout、block ids 和 MR 描述符；
- P/D 完成信号走 RDMA IMM，D 侧后台 waiter 收到 IMM 后由 `get_finished` 上报给 vLLM。
...
D 在收到最后一层 IMM 后才放行 decode。当前实现用后台 RDMA waiter 等 IMM，
`get_finished` 把完成状态上报给 vLLM scheduler；`wait_for_layer_load` 不再承担同步等待，
避免把 engine loop 卡在第一层 hook。
...
P 主线程不能在 `save_kv_layer` / `wait_for_save` 里同步等 RDMA。等待要全部挪到后台，
`get_finished` 通过 vLLM 给的 `finished_req_ids` 做交集，只在 vLLM 真要释放 req 时才回收
本地 MR/handshake——这同时保证 P 的 KV cache 在还被后台 RDMA 引用时不会被覆盖。

参考 `pegaflow/connector/worker.py` 的 `_req_pending_saves` / `_completed_saves` /
`finished_req_ids` 三集合交语义。pd push 套到这套模型上：

```
save_kv_layer (主线程):
  - 提交 push (已经异步 via _push_sender) ✓
  - 不要 wait_for_pushes
  - 不要直接调 push_done
...
last layer of last chunk (主线程):
  ...
  - finalize 后台线程：
      wait_for_pushes(req_id)   # 等所有 scatter WRITE 落地（多 NIC 必须）
...
要点：
- `wait_for_pushes` 不能砍：多 NIC scatter 跨 QP 不保序，IMM 必须等所有 WRITE 落地才发；
  但这次等待要在后台线程做，不在 forward 路径上。
- `finished_req_ids & _completed_pushes` 是正确性而不是优化：早回 `finished_sending` 会让
  vLLM 立刻 `request_finished` 回收 KV 显存，后台 RDMA 会读到脏数据。
- `wait_for_save` 退化为 no-op（或 shutdown 时的 sanity drain）。
...
- `rdma.py` 定义 `RdmaPort`、测试用 `MockRdmaPort` 和生产用 `RealRdmaPort`；
...
| `wait_for_layer_load(name)` | 不调用 | 只做 layout assert / drain；RDMA done 由后台 waiter + `get_finished` 异步上报 |
| `save_kv_layer(name, kv, ...)` | 见 §3.7 推送逻辑 + §3.11 完成上报：只**提交**异步 push，不阻塞；最后一个 chunk 的最后一层只把 req_id 丢进 `_req_pending_finalize`，由后台线程 `wait_for_pushes` + `push_done`(IMM) | 不调用 |
| `wait_for_save` | no-op（或 shutdown 时 sanity drain），见 §3.11 | 不调用 |
| `get_finished(finished_req_ids)` | 见 §3.11：`finished_req_ids & _req_pending_finalize & _completed_pushes` 才回 `finished_sending`；超时失败的 req 也走这条路径上报 | 后台 `wait_done` 完成的 req 进 `_completed_recvs`；同样和 `finished_req_ids` 取交集后回 `finished_recving`；超时走对称失败上报触发 D 本地重算 |
...
│           ├── rdma.py                # RdmaPort protocol + Mock/Real implementation
...
当前可以不接真实 router，直接向 D 注入 `kv_transfer_params`。proxy 只作为手动构造
请求的调试工具；生产语义是 Router 只请求 D，D 再向 P 发 prefill 请求。
...
2. proxy 只向 D 发 decode 请求，注入 `do_remote_prefill=true` 和 `prefill_url`；
3. D 的 connector allocate KV blocks 后进入 async load，并用 proxy 给的 P hint 触发 P prefill；
4. P 的 connector 在最后一层覆盖完目标 blocks 后发送 RDMA IMM；
5. D 的后台 RDMA waiter 收到 IMM 后在 `get_finished` 里上报 `finished_recving`，vLLM 才继续 decode。

验收先看三份日志：
- `proxy.log`：应出现 `request=... -> D`、`D completed`，不应出现 `request=... -> P`；
- `p.log`：应出现 `P queued async push`、`P finished RDMA push`；
- `d.log`：应出现 `D queued async wait`、`D -> P prefill request`、`D -> P prefill completed`、`D received RDMA done`、`scheduler finished recving`。
...
9. **MLA KV layout 支持**：Kimi 系列和不少新模型使用 MLA，KV cache 形态不是 FlashAttention
