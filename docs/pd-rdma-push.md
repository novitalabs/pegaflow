...
```python
class PdRdmaEngine:
    def __init__(self, *, cuda_device: int, numa_node: int | None = None) -> None: ...
    ...
    def push_done(self, req_id: str) -> None: ...
    def poll_finished_sending(self) -> set[str]: ...
    def poll_finished_recving(self) -> set[str]: ...
    def poll_failed_blocks(self) -> set[int]: ...
...
| **P2** 明确 Python/Rust binding surface | 根据 P1 的 `RdmaPort` 调用点，整理最小 Rust API；第一版走 PyO3 `PdRdmaEngine` | 文档列出 `register_local_layers/register_remote/push_layer/push_done/wait_done/pop_finished_*` 的真实参数和返回值 |
...
当前 P1 已经有 skeleton：`pegaflow.pd_connector`、FlashAttention HND layout assert、mock/noop RDMA port、D handshake 发布、P 按 layer/block push、最后一层所有 block 推完后 done。下一步进入 P2：写 PyO3 `PdRdmaEngine`，把 `RdmaPort` 的窄接口接到 `pegaflow-transfer::v2`。
