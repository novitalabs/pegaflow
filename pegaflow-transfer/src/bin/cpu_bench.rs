//! CPU-memory RDMA benchmark for the transfer engine.
//!
...
use pegaflow_transfer::{
    ...
    MemoryRegionDescriptor, MemoryRegionHandle, RdmaEngine, SingleTransferRequest, TopologyGroup,
    TransferEngine, TransferEngineBuilder, TransferRequest, detect_topology, init_logging,
};
...
#[command(
    ...
    version,
    about = "RDMA CPU-memory PD push benchmark"
)]
struct Cli {
...
fn main() {
    ...
    println!(
        "pegaflow-cpu-bench: memory={:?} block_size={} blocks_per_task={} tasks={} warmup={} pool_size={} hugepages={}",
        cli.memory,
        cli.block_size,
        cli.blocks_per_task,
        cli.tasks,
    ...
    let topology = detect_topology().expect("topology detection failed");
    ...
    if !selected_any {
        eprintln!("error: no topology groups matched the selected filters");
        std::process::exit(1);
    }
...
fn create_engine_context(
    group: &TopologyGroup,
    domains: Vec<pegaflow_transfer::DomainInfo>,
    memory: MemoryKind,
    buf_size: usize,
    use_hugepages: bool,
) -> EngineContext {
...
fn build_engine(
    ...
    builder.build().expect("transfer engine build failed")
}
    ...
    for (i, &nblocks) in schedule.iter().enumerate() {
        ...
        for &offset in &offsets {
            ...
            ctx.prefill
                ...
                .expect("Single transfer submit failed");
        }
        ...
        ctx.prefill
            ...
            .expect("Imm transfer submit failed");
        imm_counter.wait(1);
...
fn print_bench_result(result: &BenchResult, block_size: usize) {
    ...
    println!();
    println!("=== PD push WRITE+IMM: {} ===", result.label);
    println!(
