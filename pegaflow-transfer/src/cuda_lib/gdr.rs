...
impl GdrCopyContext {
    pub fn new() -> GdrResult<Self> {
        ...
        Ok(GdrCopyContext {
            context: Arc::new(GdrContextHandle { handle }),
        })
    }
    ...
    fn alloc_buffer(&self, nbytes: usize) -> GdrResult<GdrBuffer> {
        ...
        let ret =
            unsafe { gdrapi_sys::gdr_pin_buffer(g, aligned_device_ptr, bytesize, 0, 0, &mut mh) };
        if ret != 0 {
...
impl<T: Sized> GdrVec<T> {
    ...
    pub fn copy(&self, value: &[T]) {
        ...
        self.buffer
            .copy_to(value.as_ptr() as *const c_void, size_of_val(value));
    }
}
