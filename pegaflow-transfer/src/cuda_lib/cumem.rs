...
impl CUMemAlloc {
    fn map(alloc: CUMemMappingRef, device: Device) -> CudaResult<CUMemMapping> {
        ...
        cuda_check!(cuda_sys::cuMemAddressReserve(
            &mut ptr,
            alloc.size(),
            0,
            0,
        ...
        let mut access_desc = unsafe { std::mem::zeroed::<cuda_sys::CUmemAccessDesc>() };
...
impl CUMemAllocHandle {
    ...
    pub fn new(
        ...
        let mut handle = unsafe { std::mem::zeroed::<cuda_sys::CUmemGenericAllocationHandle>() };
    ...
    pub fn export(&self) -> CudaResult<CUMemExportHandle> {
        Ok(CUMemExportHandle(
            CUMemExport {
                handle: GenericExportHandle::new(self.handle.handle_kind, self.handle.handle)?,
...
impl CUMemMapping {
    ...
    pub fn unmap(&self) -> CudaResult<()> {
        if self.handle.mapped.swap(false, Ordering::Relaxed) {
            cuda_check!(cuda_sys::cuMemUnmap(
                self.handle.ptr,
                self.handle.alloc_size
            ))?;
        }
...
impl GenericExportHandle {
    ...
    fn bind(&self) -> CudaResult<cuda_sys::CUmemGenericAllocationHandle> {
        match self {
            GenericExportHandle::ExportedFD { .. } => Err(CudaError::CustomError(
                "Cannot bind exported FD".to_string(),
            )),
            GenericExportHandle::ImportedFD { local_fd } => {
                let mut handle =
...
impl Encode for GenericExportHandle {
    fn encode<E: Encoder>(&self, encoder: &mut E) -> Result<(), EncodeError> {
        match self {
            ...
            GenericExportHandle::ExportedFD { fd, pid } => {
                Encode::encode(&GenericSerializedHandle::FD { fd: *fd, pid: *pid }, encoder)
            }
            GenericExportHandle::Fabric { handle } => {
    ...
    fn decode<D: Decoder>(decoder: &mut D) -> Result<Self, DecodeError> {
        ...
        match payload {
            GenericSerializedHandle::FD { fd, pid } => {
                ...
                let local_fd = unsafe { libc::syscall(libc::SYS_pidfd_getfd, pidfd, fd, 0) } as i32;
                if local_fd < 0 {
            ...
            GenericSerializedHandle::Fabric(data) => {
                ...
                let mut handle = unsafe { std::mem::zeroed::<cuda_sys::CUmemFabricHandle>() };
                unsafe {
...
impl CUMulticastHandle {
    pub fn new(num_devices: u32, size: usize, handle_kind: CUMemHandleKind) -> CudaResult<Self> {
