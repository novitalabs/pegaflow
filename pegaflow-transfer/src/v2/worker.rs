...
use crate::v2::{
    api::{
        ...
        TransferCompletionEntry, TransferCounter, TransferId, TransferRequest, UvmWatcherId,
    },
...
pub enum UvmWatcherCall {
    AcquireUvmWatcher {
        ret: oneshot::Sender<Option<UvmWatcherId>>,
    },
    ReleaseUvmWatcher {
        watcher: UvmWatcherId,
        ret: oneshot::Sender<()>,
    },
}
...
impl WorkerHandle {
    ...
    pub fn join(self) {
        ...
        self.uvm_handle
            .join()
            .expect("Failed to join UVM watcher thread");
    }
}
...
impl InitializingWorker {
    pub fn wait_init(self) -> Result<WorkerHandle> {
        ...
        let init_uvm = self
            .init_uvm_rx
            .recv()
            .map_err(|_| FabricLibError::Custom("Failed to receive UVM watcher init"))?;
...
impl UvmWatcherContext {
    fn new() -> Self {
        ...
        let uvm_memory =
            CudaHostMemory::alloc(UVM_WATCHER_BYTES).expect("Failed to allocate UVM memory");
    ...
    fn release(&mut self, id: UvmWatcherId) {
        ...
        let slot =
            (ptr.as_ptr() as usize - self.uvm_memory.ptr.as_ptr() as usize) / size_of::<u64>();
...
fn uvm_worker_thread(
    ...
    // Initialization complete
    let stop_signal = Arc::new(AtomicBool::new(false));
    let init = InitializedUvmWatcher {
        stop_signal: stop_signal.clone(),
        call_tx,
    };
    if init_tx.send(Ok(init)).is_err() {
    ...
    if let Some(uvm_ctx) = uvm_ctx {
        for slot in uvm_ctx.used_slots.iter() {
            ...
            if value != old {
                ...
                let comp = TransferCompletionEntry::UvmWatch {
                    id,
                    old,
                    new: value,
                };
