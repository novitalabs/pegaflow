...
impl TransferEngine {
    ...
    pub fn set_imm_count_expected(
        ...
        self.callbacks
            .imm_count
            .insert(imm, ImmCountFn::Repeated(callback));
...
impl SendRecvEngine for TransferEngine {
    fn submit_send(
        ...
        self.engine
            .submit_send(transfer_id, addr, buffer.mr_handle, buffer.ptr, buffer.len)?;
        Ok(())
    }
    ...
    fn submit_bouncing_recvs(
        ...
        let storage: Arc<Vec<MaybeUninit<u8>>> = Arc::new(Vec::with_capacity(len * count));
        let buf_base = unsafe { NonNull::new_unchecked(storage.as_ref().as_ptr() as *mut c_void) };

        // Register memory regions
        let mr_handle = self
            .engine
            .register_memory_local(buf_base, len * count, Device::Host)?;
        ...
        for i in 0..count {
            ...
            let on_recv_wrapper = Box::new(move |data_len: usize| {
                ...
                let data =
                    unsafe { std::slice::from_raw_parts(ptr.as_ptr() as *const u8, data_len) };

                on_recv_ref(data)?;
                Ok(())
            });

            let on_error_wrapper = { Box::new(move |e: FabricLibError| on_error_ref(e)) };
...
impl AsyncTransferEngine for TransferEngine {
    async fn wait_for_imm_count(&self, imm: u32, expected_count: NonZeroU32) -> Result<()> {
        ...
        let callback = Box::new(move || {
            tx.send(Ok(()))
                .expect("Failed to send through oneshot channel");
        });

        tokio::task::block_in_place(move || {
            self.callbacks
                .imm_count
                .insert(imm, ImmCountFn::Once(callback));
            self.engine.set_imm_count_expected(imm, expected_count);
        });
    ...
    async fn submit_send_async(&self, addr: DomainAddress, buffer: SendBuffer) -> Result<()> {
        let (tx, rx) = oneshot::channel();
