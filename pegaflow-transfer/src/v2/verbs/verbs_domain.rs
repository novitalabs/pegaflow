...
impl VerbsDomain {
    fn open(info: VerbsDeviceInfo, imm_count_map: Arc<ImmCountMap>) -> Result<Self> {
        unsafe {
            ...
            let td = NonNull::new(ibv_alloc_td(context.as_ptr(), &raw mut td_init_attr))
            ...
            let rma_srq = NonNull::new(ibv_create_srq(pd.as_ptr(), &raw mut srq_init_attr))
                .ok_or_else(|| VerbsError::with_last_os_error("ibv_create_srq"))?;
            ...
            let ud_mempool = MemoryPool::new(MAX_UD_MSG_BYTES, NUM_UD_RECVS + MAX_UD_SENDS);
            let mut domain = Self {
                name,
    ...
    fn post_imm_recv(&mut self) -> Result<()> {
        ...
        let ec = unsafe { ibv_post_srq_recv(self.rma_srq.as_ptr(), &raw mut wr, null_mut()) };
        if ec != 0 {
    ...
    fn do_submit_write<F: FnOnce(*mut c_void, NonNull<WrChainBuffer>) -> WriteOpIter>(
        &mut self,
        transfer_id: TransferId,
    ...
    fn progress_rdma_recv_ops(&mut self) {
        ...
        while let Some(ctx) = self.recv_ops.front() {
            ...
            let ret = unsafe {
                ibv_post_srq_recv(self.msg_srq.as_ptr(), wr.as_mut_ptr(), bad_wr.as_mut_ptr())
            };
            match ret {
                0 => {
    ...
    fn progress_rdma_send_ops(&mut self) {
        ...
        while let Some(ctx) = self.send_ops.front() {
            ...
            let ret =
                unsafe { ibv_post_send(ctx.msg_qp.as_ptr(), wr.as_mut_ptr(), bad_wr.as_mut_ptr()) };
            match ret {
                0 => {
        ...
        match wc.opcode {
            ...
            IBV_WC_RECV_RDMA_WITH_IMM => {
                ...
                match self.imm_count_map.inc(imm) {
                    ...
                    ImmCountStatus::Reached => Some(DomainCompletionEntry::ImmCountReached(imm)),
                }
            }
            _ => {
                warn!(
...
impl RdmaDomain for VerbsDomain {
    ...
    fn get_mem_desc(&self, ptr: NonNull<c_void>) -> Result<MemoryRegionLocalDescriptor> {
        let mr = self
            .local_mr_map
            .get(&ptr)
            .ok_or(FabricLibError::Custom("Local MR not found"))?;
        Ok(MemoryRegionLocalDescriptor(
            unsafe { mr.as_ref() }.lkey as u64,
        ))
    }
    ...
    fn submit_send(&mut self, transfer_id: TransferId, dest_addr: DomainAddress, op: SendOp) {
    ...
    fn add_peer_group(&mut self, handle: PeerGroupHandle, addrs: Vec<DomainAddress>) -> Result<()> {
    ...
    fn submit_group_write(
        ...
        let rma_qps = if let Some(handle) = handle {
            if let Some(connecting_peer_group) = self.connecting_peer_groups.get_mut(&handle) {
                connecting_peer_group
                    .pending_group_ops
                    .push((transfer_id, op));
                return;
            }
