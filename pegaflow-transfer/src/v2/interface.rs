...
impl SendBuffer {
    pub fn new(ptr: NonNull<c_void>, len: usize, mr_handle: MemoryRegionHandle) -> Self {
        Self {
            ptr,
            len,
            mr_handle,
...
pub type ErrorCallback = Box<dyn FnOnce(FabricLibError) -> CallbackResult + Send + Sync>;

pub type BouncingRecvCallback = Arc<Box<dyn Fn(&[u8]) -> CallbackResult + Send + Sync>>;

pub type BouncingErrorCallback = Arc<Box<dyn Fn(FabricLibError) -> CallbackResult + Send + Sync>>;
