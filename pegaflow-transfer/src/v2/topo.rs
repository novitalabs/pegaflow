...
impl FromStr for PciAddress {
    ...
    fn from_str(addr: &str) -> std::result::Result<PciAddress, &'static str> {
        ...
        if !(&addr[4..5] == ":" && &addr[7..8] == ":" && &addr[10..11] == ".") {
            return Err("Invalid PCI address format, expected domain:bus:device.function");
        }

        let domain = u16::from_str_radix(&addr[0..4], 16).map_err(|_| "Invalid domain")?;
        let bus = u8::from_str_radix(&addr[5..7], 16).map_err(|_| "Invalid bus")?;
        let device = u8::from_str_radix(&addr[8..10], 16).map_err(|_| "Invalid device")?;
        let function = u8::from_str_radix(&addr[11..12], 16).map_err(|_| "Invalid function")?;
...
impl From<&VerbsDeviceInfo> for PciAddress {
    fn from(domain: &VerbsDeviceInfo) -> Self {
        ...
        PciAddress {
            ...
            device: u8::from_str_radix(&pci_addr[8..10], 16).expect("Failed to parse device"),
            function: u8::from_str_radix(&pci_addr[11..12], 16).expect("Failed to parse function"),
        }
    }
}
...
fn get_numa_physical_cpus() -> Result<Vec<Vec<u16>>> {
    ...
    for entry in std::fs::read_dir("/sys/devices/system/node")
        .map_err(|_| FabricLibError::Custom("Failed to read /sys/devices/system/node"))?
    {
        let entry = entry
            .map_err(|_| FabricLibError::Custom("Failed to read /sys/devices/system/node entry"))?;
        ...
        let cpulist = std::fs::read_to_string(entry.path().join("cpulist")).map_err(|_| {
            FabricLibError::Custom("Failed to read /sys/devices/system/node?/cpulist")
        })?;
    ...
    for line in cpuinfo.lines() {
        ...
        match key {
            ...
            "physical id" => {
                physical_id = value
                    .parse::<u16>()
                    .map_err(|_| FabricLibError::Custom("Failed to parse physical id"))?;
            }
            "core id" => {
                let core_id = value
                    .parse::<u16>()
...
fn scan_all_pci_devices() -> Result<Vec<PciProp>> {
    ...
    {
        let entry = entry.map_err(|_| FabricLibError::Custom("Failed to read PCI entry"))?;
        ...
        let mut parent_bus = parent_addr_str
            .parse::<PciAddress>()
            .map_err(|_| FabricLibError::Custom("Failed to parse parent PCI address"))?;
        parent_bus.device = 0;
