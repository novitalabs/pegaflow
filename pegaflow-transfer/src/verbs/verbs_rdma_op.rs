...
impl SingleWriteOpIter {
    pub(super) fn new_single(
        ...
        let sge = sge.write(ibv_sge {
            addr: unsafe { op.src_ptr.as_ptr().byte_add(op.src_offset as usize) } as u64,
    ...
    pub(super) fn new_imm(
        ...
        wr.imm_data_invalidated_rkey_union.imm_data = op.imm_data.to_be();
...
impl PagedWriteOpIter {
    pub(super) fn new(
        ...
        // Prepare WR template
        let (opcode, imm) = opcode_imm(op.imm_data);
        let chain_len = std::cmp::min(op.page_indices_end - op.page_indices_beg, WR_CHAIN_LEN);
        let buf = unsafe { wr_chain_buffer.as_mut() };
    ...
    pub(super) fn peek(&self) -> (*mut ibv_qp, *mut ibv_send_wr, usize) {
        ...
        (
            self.rma_qp.as_ptr(),
            buf[self.i_wr_head].0.as_mut_ptr(),
            self.wr_len,
        )
    }
    ...
    fn fill_wr(&mut self) {
        ...
        sge.addr = unsafe {
            self.src_ptr
                .as_ptr()
                .byte_add(self.src_stride as usize * src_page_idx)
        } as u64;
...
fn opcode_imm(imm_data: Option<u32>) -> (u32, u32) {
    if let Some(imm_data) = imm_data {
        (ibv_wr_opcode::IBV_WR_RDMA_WRITE_WITH_IMM, imm_data)
    } else {
        (ibv_wr_opcode::IBV_WR_RDMA_WRITE, 0)
    }
}
