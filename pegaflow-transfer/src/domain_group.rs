...
use crate::{
    ...
    rdma_op::{
        GroupWriteOp, ImmWriteOp, PagedWriteOp, RecvOp, ScatterGroupWriteOp, SendOp, SingleWriteOp,
        WriteOp,
    },
};
...
impl<D: RdmaDomain, const N: usize> DomainGroup<D, N> {
    ...
    pub(crate) fn register_mr_allow_remote(
        ...
        let remote = MemoryRegionDescriptor {
    ...
    pub(crate) fn submit_imm_transfer_request(
        ...
        let (first_domain, imm_per_domain) = match domain {
            DomainGroupRouting::RoundRobinSharded { num_shards } => {
                ...
                self.rr_next = (self.rr_next + num_domains.min(dst_mrs.len())) % num_domains;
    ...
    pub(crate) fn submit_paged_transfer_request(
        ...
        let page_range = divide_evenly(request.src_page_indices.len(), self.domains.len());
...
fn shard_single_transfer(total_size: usize, num_shards: usize) -> SmallVec<(usize, usize)> {
